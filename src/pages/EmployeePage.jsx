...
const EmployeesPage = () => {
  ...
  const positionOptions = ['Senior Developer', 'Human Resource Lead', 'Designer', 'Full Stack Developer', 'Frontend Developer', 'Backend Developer', 'Product Manager', 'Intern'];
  ...
  useEffect(() => {
    const fetchEmployees = async () => {
      try {
        ...
        setError('Failed to fetch employees. Please try again or check your authentication.');
  ...
  const validateEditedEmployee = () => {
    const errors = {};
    if (!editedEmployee.fullName.trim() || editedEmployee.fullName.length < 2) errors.fullName = 'Full name must be at least 2 characters';
    if (!/^\S+@\S+\.\S+$/.test(editedEmployee.email)) errors.email = 'Invalid email format';
    if (!/^\+?[\d\s-]{10,15}$/.test(editedEmployee.phone)) errors.phone = 'Invalid phone number (10-15 digits)';
    if (!editedEmployee.position) errors.position = 'Position is required';
    if (!editedEmployee.department) errors.department = 'Department is required';
    if (!editedEmployee.dateOfJoining) errors.dateOfJoining = 'Date of Joining is required';
    return errors;
  };
  ...
  const handleEditEmployee = async (e) => {
    ...
    try {
      setIsLoading(true);
      setError(null);
      const response = await axios.patch(`${BASE_API_URL}/employees/${employeeToEdit._id}`, editedEmployee, {
        headers: { Authorization: `Bearer ${token}` }
      });
      setEmployees(employees.map(emp => emp._id === employeeToEdit._id ? response.data : emp));
      setShowEditModal(false);
      setEmployeeToEdit(null);
    } catch (error) {
      setError('Failed to update employee. Please try again or check your authentication.');
  ...
  const handleDeleteEmployee = async (employeeId) => {
    try {
      ...
      setError('Failed to delete employee. Please try again or check your authentication.');
  ...
  return (
    <div className="employee-container">
      ...
      {isLoading && <div className="loading-message" style={{ textAlign: 'center', marginBottom: '1rem' }}>Loading...</div>}
            ...
            {!isLoading && filteredEmployees.length === 0 ? (
              <tr>
                <td colSpan="8" style={{ textAlign: 'center', padding: '2rem' }}>
                  No employees found. {error ? 'Please resolve the error and try again.' : 'Try adjusting the filters.'}
                </td>
              </tr>
            ) : (
        ...
        <div className="modal-overlay" onClick={() => setShowEditModal(false)}>
          <div className="modal-content" onClick={(e) => e.stopPropagation()}>
            ...
            <form onSubmit={handleEditEmployee}>
              ...
              <button type="submit" className="submit-btn" disabled={isLoading}>Save Changes</button>
            </form>
          </div>
        </div>
      )}
        ...
        <div className="modal-overlay" onClick={() => setShowDeleteModal(false)}>
          <div className="modal-content delete-modal" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="delete-content">
              <p>Are you sure you want to delete {employeeToDelete?.fullName}?</p>
